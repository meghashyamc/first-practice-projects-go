...
//KIPError generates error log
func KIPError(requestID string, e error, data LogDataMap) {
...
//KIPInfo generates info log
func KIPInfo(requestID, infoMessage string, data LogDataMap) {
	trackingIDs := generateTrackingIDs(requestID)
	dataBytes, _ := json.Marshal(data)
	dataString := string(dataBytes)
...
//KIPWarning generates warning log
func KIPWarning(requestID, warnMessage string, data LogDataMap) {
	trackingIDs := generateTrackingIDs(requestID)
	msg := fmt.Sprintf("|%s|", trackingIDs)
	if data != nil && len(data) > 0 {
		dataBytes, _ := json.Marshal(data)
...
//KIPTrace generates trace log
func KIPTrace(requestID, traceMessage string, data LogDataMap) {
	trackingIDs := generateTrackingIDs(requestID)
	msg := fmt.Sprintf("|%s|", trackingIDs)
	if data != nil && len(data) > 0 {
		dataBytes, _ := json.Marshal(data)
